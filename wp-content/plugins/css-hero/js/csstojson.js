var CSSJSON = new function () {
    ...
    base.toJSON = function (cssString, args) {
	    ...
	    if (cssString.indexOf('http://')>-1 || cssString.indexOf('https:')>-1){
		//    var cssString = cssString.replace(/http:/g,'[--HTTP--]')
		  //  var cssString = cssString.replace(/https:/g,'[--HTTPS--]')
	    }
	    
	    var cssString = cssString.split('*/').join('/*;')
    ...
    base.toCSS = function (node, depth, breaks) {
        ...
        // ALE
        for (var t in node){
	        if (t != 'attributes' && t != 'children'){
		        cssString += strHero(node[t],depth)
	        }
        }
        // END ALE
        if (node.attributes) {
	...
	// ALE
	var strHero = function (name, depth) {
